...
smplStatus_t nwk_nwkInit(uint8_t (*f)(linkID_t))
{
  ...
  if (CONNSTATE_FREE == sPersistInfo.connStruct[NUM_CONNECTIONS].connState)
  {
	...
	if (CONNSTATE_FREE == sPersistInfo.connStruct[NUM_CONNECTIONS - 1].connState)
  {
    sPersistInfo.connStruct[NUM_CONNECTIONS - 1].connState   = CONNSTATE_CONNECTED;
    sPersistInfo.connStruct[NUM_CONNECTIONS - 1].hops2target = MAX_HOPS;
    sPersistInfo.connStruct[NUM_CONNECTIONS - 1].portRx      = SMPL_PORT_USER_LINEAR;
    sPersistInfo.connStruct[NUM_CONNECTIONS - 1].portTx      = SMPL_PORT_USER_LINEAR;
    sPersistInfo.connStruct[NUM_CONNECTIONS - 1].thisLinkID  = SMPL_LINKID_LINEAR;
  }
...
uint8_t nwk_isConnectionValid(mrfiPacket_t *frame, linkID_t *lid)
{
  ...
  for (i=0; i<SYS_NUM_CONNECTIONS; ++i,++ptr)
  {
    if (CONNSTATE_CONNECTED == ptr->connState)
    {
      ...
      if (port == ptr->portRx)
      {
				/* no check peer addr @yongfa */
				if(port == SMPL_PORT_USER_LINEAR)
				{
